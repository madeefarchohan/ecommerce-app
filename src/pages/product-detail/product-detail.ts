...
export class ProductDetailPage {
  ...
  share() {
    ...
    if (this.product.type == "grouped")
      for (let val of this.groupProducts) { images.push(val.images[0].src); }

    this.loading.autoHide(5000);
    // Share via email
    this.socialSharing.share(
      "",//this.product.short_description,   //The message you would like to share.
  ...
  // <!-- 2.0 updates -->
  getRelatedItems() {
    this.config.Woocommerce.getAsync("products/?include=" + this.product.related_ids).then(data => {
  ...
  openStore() {
    this.loading.show();
    this.config.dokan.getAsync("stores/" + this.product.store.id).then(data => {
      this.loading.hide();
      let d = JSON.parse(data.body);
